...
- The sample comes from a **normally distributed population**, or the **sample size is large** (typically \( n \geq 30 \)).
...
Where:
- \( s \) is the sample standard deviation (used instead of \( \sigma \))
...
Use this test when:
- The variable is **binary** (success/failure, yes/no, etc.), and
- You want to test a **population proportion** \( p \), using a large enough sample.
