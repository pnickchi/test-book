...
ldl_after <- ldl_before - Delta + rnorm(n_patients, mean = 0, sd = 5)
...
$$H_0: \mu_d = 0 \quad \text{(no mean change in LDL levels)}$$

$$H_a: \mu_d \neq 0 \quad \text{(mean LDL levels change after treatment)}$$
...
# Run paired t-test
result <- t.test(x = ldl_before, y = ldl_after, paired = TRUE, alternative = "two.sided")
...
- `alternative`: specifies the type of test; "two.sided" tests for any change in LDL.

The output includes:

- `statistic`: the calculated t-test statistic.

- `p.value`: the p-value for evaluating the significance of the observed difference.

- `conf.int`: confidence interval for the mean difference in LDL levels.

- `estimate`: the average LDL levels before and after treatment.
