...
#| fig-cap: "Side-by-side histogram of wage by education level"
#| warning: false
#| echo: false
#| fig.height: 8
#| fig.width: 14
...
For instance imagine you are investigating support for a new policy among two different regions. Suppose a national statistics office previously carried out large-scale surveys and suggested that the variability in responses for each region can be reasonably modeled using the binomial distribution framework. Your aim is not the variance but rather the proportion of individuals in each group who favor the policy.

Specifically, let’s consider two independent populations:
...
The main statistical question becomes: **Is there a significant difference between the true population proportions of support in the two regions?** Formally, we test the hypotheses:
...
This leads us to the two-sample z-test for population proportions which provides a way to formally compare the two groups using their observed sample proportions.
