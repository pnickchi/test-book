...
Note that `cholesterol_data` contains two columns: `LDL_before`, which records each patient's LDL level before taking the new medication, and `LDL_after`, which records the LDL level after taking the new medication. This setup clearly illustrates the paired nature of the data: each row corresponds to a single individual, with measurements taken at two different points in time: **before** and **after** the treatment. Because the observations are linked within each person, the analysis must account for this pairing rather than treating the two sets of values as independent samples. Once we have dataset collected, we can now move on to some EDA.

Having the dataset in its current wide format makes it easy to see the paired nature of the measurements, but this structure is not ideal for the next steps of our analysis especially for exploratory data analysis (EDA). To prepare the data for plotting and statistical testing, we first need to reshape it into a long format. The `pivot_longer` function from the `tidyverse` is the key tool for this transformation.

```{r}
cholesterol_long <- cholesterol_data %>% 
  pivot_longer(cols = c(LDL_before, LDL_after),
               names_to = 'group',
               values_to = 'LDL')
```
...
```{r}
head(cholesterol_long, n = 6)
```


**Note:** This format is great for plotting in `ggplot2` or running paired analyses later.


### Exploratory Data Analysis

```{r}
#| label: fig-density-plot-ldl-before-after
#| fig-cap: "Density Plot of LDL Levels Before and After Treatment"
#| warning: false
#| echo: false
#| fig.height: 8
#| fig.width: 14
ggplot(cholesterol_long, aes(x = LDL, fill = group)) +
  geom_density(alpha = 0.4) +
  labs(title = '',
    x = 'LDL Level (mg/dL)',
    y = 'Density') +
