...
Both code snippets below (in `R` and `Python`) take the data frame `OneWay_ABn_training` and compute descriptive summaries of the continuous outcome variable `conversion_score`, grouped by the categorical variable `webpage_design`. The computations include three basic summary measures: 

1. the **mean conversion score** (i.e., average performance of each webpage design), 
2. the **standard deviation** (i.e., the within-group variability in customer conversion behaviour), and 
3. the **sample size `n`** (i.e., number of observations per treatment).

These summaries provide a quick, exploratory understanding of the data, helping us obtain descriptive insights into the central tendency and spread of our outcome of interest per experimental treatment. The `R` implementation uses the [{tidyverse}](https://tidyverse.org/) meta-package. Conversely, the `Python` implementation uses the [{pandas}](https://pandas.pydata.org/) library for these numerical computations.
...
:::
::: Heads-up-container
In the below `Python` code snippet, we are importing the `OneWay_ABn_training` set, which was created from our random data splitting in the `R` environment using the [{reticulate}](https://rstudio.github.io/reticulate/) [@reticulate] package. We will eventually also do the same with the `OneWay_ABn_testing` dataset. This approach is necessary because both the [{rsample}](https://rsample.tidymodels.org) package in `R` and [{scikit-learn}](https://scikit-learn.org/stable/) in `Python` utilize different pseudo-random number generators. As a result, they produce different training and testing data splits, even when using the same seed values. Therefore, to ensure consistent output across all subsequent `R` and `Python` snippets in our workflow, it is necessary that we use the same training and testing sets.
:::
:::::
...
The mean conversion scores (found in both @tbl-one-summary-stats-r and @tbl-one-summary-stats-py) are **very similar across the three webpage designs**, ranging narrowly from `r min(OneWay_summary_table$mean_conversion)` to `r max(OneWay_summary_table$mean_conversion)`. Additionally, **the standard deviations are also alike**, with values ranging from `r min(OneWay_summary_table$sd_conversion)` to `r max(OneWay_summary_table$sd_conversion)`. These standard deviations suggest similar levels of variability within each webpage. Although there are minor differences in the treatment means, they are subtle enough that formal hypothesis testing, such as one-way ANOVA, is necessary to determine whether these small fluctuations are due to random variation or represent genuine differences in the population means. These summary statistics therefore provide a solid descriptive check on group comparability and the plausibility of ANOVA assumptions, which sets the stage for **a rigorous inferential analysis**.

We will use two plots in our EDA: **three overlaid histograms** (one per treatment) to compare the continuous conversion scores and **three side-by-side boxplots** of these scores per treatment. We use `R`'s [{ggplot2}](https://ggplot2.tidyverse.org/), from [{tidyverse}](https://tidyverse.org/), along with `Python`'s [{matplotlib}](https://matplotlib.org/) and [{seaborn}](https://seaborn.pydata.org/). The overlaid histograms in @fig-hist-webpage-design-one-r and @fig-hist-webpage-design-one-py indicate the following:

- The three webpage designs exhibit approximately **unimodal** and **roughly symmetric distributions** centred around conversion scores in the low-60s, with slight differences in spread. 
- Design $D_1$ appears slightly more concentrated around its mean, while $D_2$ and $D_3$ show marginally wider dispersion, reflected in slightly flatter histogram peaks.
- The overlapping structure suggests that the central tendency (i.e., mean conversion score) is quite similar across the three webpage designs, with no visually apparent separation.
...
```{r}
#| label: fig-hist-webpage-design-one-r
#| fig-cap: "Overlaid histograms of customer conversion score by webpage design via training data."
#| warning: false
#| message: false
#| fig.height: 8
#| fig.width: 14
...
```{python}
#| label: fig-hist-webpage-design-one-py
#| fig-cap: "Overlaid histograms of customer conversion score by webpage design via training data."
#| warning: false
#| message: false
#| fig.height: 8
...
# Overlaying histograms with transparency and white edges
for label, color in palette.items():
    subset = OneWay_ABn_training[OneWay_ABn_training["webpage_design"] == label]
    plt.hist(
        subset["conversion_score"],
        bins=30,
        alpha=0.7,
        color=color,
        edgecolor="white",
...
plt.xlabel("Conversion Score", fontsize=20)
...
# Removing top/right spines and adjust layout
sns.despine()
plt.tight_layout()
...
Now, in  @fig-boxplot-webpage-design-one-r and @fig-boxplot-webpage-design-one-py each boxplot represents the distribution of customer conversion scores for one of the three webpage-design conditions ($D_1$, $D_2$, and $D_3$). The central line in each box is the median score; the top and bottom of the box span the 25th to 75th percentiles (the interquartile range, IQR); and the whiskers extend to the non-outlier extremes (according to the conventional 1.5× IQR rule). From a visual comparison of the three groups, we have the following:

- The medians of $D_1$, $D_2$, and $D_3$ appear **quite similar**, indicating that the central tendency of conversion score is roughly equivalent across the three webpage designs.
- The box heights (i.e., IQRs) show that **the spread of the middle 50% of scores is also very similar across experimental treatments**, meaning no webpage design shows substantially more variability in the core of its distribution than the others.
- The whiskers and any outlier points (dots beyond whiskers) may reveal slight differences. For example, if one design has a somewhat longer upper whisker, that signals a larger tail of higher conversion scores in that condition.
...
```{r}
#| label: fig-boxplot-webpage-design-one-r
#| fig-cap: "Side-by-side boxplots of customer conversion score by webpage design via training data."
#| warning: false
#| message: false
#| fig.height: 8
#| fig.width: 14
...
```{python}
#| label: fig-boxplot-webpage-design-one-py
#| fig-cap: "Side-by-side boxplots of customer conversion score by webpage design via training data."
#| warning: false
#| message: false
#| fig.height: 8
...
ax.set_xlabel("Webpage Design", fontsize=20)
...
The combined histograms and side-by-side boxplots indicate that there is **no strong visual separation** among the distributions of customer conversion scores for the three webpage designs. The central tendency, spread, and tails of the distributions **overlap significantly**, suggesting that any treatment effect (if it exists) is likely to be subtle rather than pronounced. In summary, the EDA evidence reveals only minor differences among the webpage designs, highlighting the need for formal statistical testing to determine whether these small observed variations are statistically significant or merely a result of random sampling variability.
