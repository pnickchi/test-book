...
We will use two plots in our EDA: **three side-by-side histograms** (one per treatment) to compare the continuous conversion scores and **three side-by-side boxplots** of these scores per treatment. We use `R`'s [{ggplot2}](https://ggplot2.tidyverse.org/), from [{tidyverse}](https://tidyverse.org/), along with `Python`'s [{matplotlib}](https://matplotlib.org/) and [{seaborn}](https://seaborn.pydata.org/). The side-by-side histograms in @fig-hist-webpage-design-one-r and @fig-hist-webpage-design-one-py indicate the following:
...
- The distributions and central tendencies (i.e., mean conversion scores) are **pretty similar** across the three webpage designs.
...
hist_one_way <- ggplot(
  ...
  facet_wrap(~ webpage_design, nrow = 1) +   # <-- horizontal panels
  scale_fill_manual(
    values = c(
      "D1" = "#E69F00",
      "D2" = "#56B4E9",
...
for ax, design in zip(axes, design_order):
    subset = OneWay_ABn_training[
        OneWay_ABn_training["webpage_design"] == design
    ]
