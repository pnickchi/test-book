...
Formally, for each participant $i$ the LDL before treatment follows a Normal distribution with mean = 160 mg/dL and a standard deviation of 4:

$$\text{LDL}_{\text{before}, i} \sim \text{Normal}(160, 4^2)$$
...
- $\varepsilon_i \sim \text{Normal}(0, 6^2)$ represents additional variation specific to the after-treatment measurement.
...
ldl_before <- rnorm(n_patients, mean = 160, sd = 4)

# Step 2: Generate LDL after treatment
ldl_after <- ldl_before - DELTA + rnorm(n_patients, mean = 0, sd = 6)
