...
This chapter covers two types of tests. First, we discuss tests for comparing the means of two samples, which are used to make inferences about population means. Second, we examine tests for comparing proportions between two samples.
...
Independent samples arise when the observations in one group do not influence or relate to the observations in the other. In statistical terms we call this two independent samples. A classic example comes from educational research:
...
$$D_i = Y_i - X_i \quad \textit{for} \quad i = 1,2, \ldots, n$$
